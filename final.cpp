...
void CannyDiscrete(CImg<float> in, float sigma, float threshold,
      ...
      CImg<float> &outNMS) {
	const int nx = in.dimx();
	const int ny = in.dimy();
...
void hough (const CImg<float>& img, 
  ...
  const int WIDTH = img.dimx();
  const int HEIGHT = img.dimy();
  ...
  const float THETA_STEP = M_PI/(float)THETA_N;
  ...
  for (unsigned i=0; i<nonmax.size(); i++) {
    ...
    /* draw line : two cases.. */
    if ((theta < M_PI/4) || (theta>(3*M_PI/4.0f))) {
...
int otsu_threshold(CImg<unsigned char> img)
{
    ...
    generate_histogram(img, histogram); // image histogram
...
int main(int argc, char * argv[])
{
    ...
    CImg<unsigned char> img("test1.png");
    ...
    gray_img.get_normalize(0,255).save("grey.jpg");
    binary_img.get_normalize(0,255).save("binary.jpg");
  ...
  CImg<float> input(binary_img);
  input = GRAY(input);
  if ((rotate != 0.0f) || (zoom!=1)) {
    input.rotate (rotate, input.dimx()/2, input.dimy()/2, zoom);
  }
  ...
  CImgl<float> imgl (RGB(input), overlay(input, output), 
                     RGB(output), RGB(houghspace));
